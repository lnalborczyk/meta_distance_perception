...
pcurve_app <- function(file1, dir1) {
    ...
    png(filename = paste(filek, ".ng", sep = ""), width = 2600, height = 2400, res = 400)  
    ...
    data.frame(x = c(".01",".02",".03",".04",".05"), obs = blue, null0 = red, null33 = green) %>%
        gather(type, value, obs:null33) %>%
        ggplot(aes(x = x, y = value, group = type, linetype = type) ) +
        geom_line(size = 0.5) +
        scale_linetype_manual(
            values = c("dotted", "dashed", "solid"),
            labels = c(" Null of no effect", " Null of 33% power", " Observed p-curve")
            ) +
        geom_point(data = . %>% filter(type == "obs") ) +
        geom_label(
            data = . %>% filter(type == "obs"),
            aes(label = value %>% round %>% paste0(., "%") ),
            nudge_y = 2.5
            ) +
    ...
    ggsave("p_curve.pdf", width = 8, height = 6, bg = "transparent")
...
file1 <- "p_curve.txt"
dir1 <- here("code", "pcurve/")
pcurve_app(file1, dir1)
